...
BOX = dict(Qs02=(0.05, 1.0), C2=(0.5, 30.0), gam=(1.0, 1.3))
BEST = dict(Qs02=0.165, C2=6.35, gam=1.135)  # MVgamma fit to HERA 2009 (x<0.01)
...
def load_bundle():
    with open(os.path.join(HERE, "mvgamma_tmd_emulator.pkl"), "rb") as f:
...
def predict_grid(qs02, c2, gam):
    ...
    """
    v = B["emu"].predict_g([[np.log(qs02), np.log(c2), gam]])[0]
...
# ── UI ────────────────────────────────────────────────────────────────────────
st.title("Small-x gluon TMD explorer  (MV\u03b3 dipole, 3 parameters)")
st.caption(
...
with cL:
    ...
    gam = st.slider(r"$\gamma$", *BOX["gam"], BEST["gam"], 0.005,
                    key="gam", help="Anomalous dimension of the MV-gamma initial condition")
    st.subheader("View")
    ...
    ref = st.checkbox("show MV\u03b3 HERA best fit (grey)", value=True)
    kfix = st.slider(r"$k_{T}$ for the evolution panel [GeV]",
                     0.5, 10.0, 1.0, 0.5)
    def _reset_to_hera():
        st.session_state["qs02"] = BEST["Qs02"]
        st.session_state["c2"] = BEST["C2"]
        st.session_state["gam"] = BEST["gam"]

    st.button("reset to HERA best fit", on_click=_reset_to_hera)

F = predict_grid(qs02, c2, gam)
Fbase = predict_grid(BEST["Qs02"], BEST["C2"], BEST["gam"])       # HERA baseline (always, for ratios)
...
with cR:
    ...
    st.caption(
        rf"$Q_{{s0}}^2 = {qs02:.3f}$ GeV$^2$, $C^2 = {c2:.2f}$, $\gamma = {gam:.3f}$  |  "
        rf"grey reference: MV$\gamma$ HERA fit ($Q_{{s0}}^2={BEST['Qs02']}$, "
        rf"$C^2={BEST['C2']}$, $\gamma={BEST['gam']}$)  |  MV$\gamma$ emulator v1 "
        "(472 training BK runs): median accuracy ~0.8%, weakest at $Y\lesssim1$ "
        "(2-3%); display uses light smoothing along $k_T$."
    )
